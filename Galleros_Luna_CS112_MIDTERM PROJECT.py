...
class TicTacToe:
    ...
    def check_win(self):
        if self.check_horizontal() or self.check_vertical() or self.check_diagonal():
            player_symbols = {' X ': self.Player1, ' 0 ': self.Player2}  # dictionary: naming the winner instead of the "X" and "0"
...
player1, player2, mode = intro()

game = TicTacToe(" X ", player1, player2, mode, board, board2)
game.intro()
game.main()
